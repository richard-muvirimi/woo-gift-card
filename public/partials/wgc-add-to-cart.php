...
<div class="wgc-options">
	...
	?>
		...
		case "selected":
			...
			<?php foreach ($prices as $price) : ?>
			    <div>
				<input type="radio" class="woocommerce-Input woocommerce-Input--text input-radio" name="wgc-receiver-price" id="wgc-receiver-price-<?php esc_attr_e($price) ?>" value="<?php esc_attr_e($price); ?>" required <?php checked($prices[0], $price); ?>/>
				<label for="wgc-receiver-price-<?php esc_attr_e($price) ?>">
				    <?php echo wc_price(wc_get_price_to_display($product, array('price' => $price))) . $product->get_price_suffix($price) ?>
				</label>
			    </div>
			<?php endforeach; ?>
	    	</div>
    ...
    <fieldset>
	<legend><?php esc_html_e("Gift Voucher recipient message", 'woo-gift-card'); ?></legend>
	...
	<?php if ($product->get_meta('wgc-schedule') == "yes"): ?>
		...
		<?php if ($product->get_meta('wgc-expiry-days') !== false): ?>
	...
	?>
        ...
        <fieldset>
    	<legend><?php esc_html_e("Details to customise the gift voucher template", 'woo-gift-card'); ?></legend>
    	...
    	<div class="woocommerce-form-row woocommerce-form-row--wide form-row form-row-wide">
			...
			<div>
			    <input type="radio" class="woocommerce-Input woocommerce-Input--text input-radio" name="wgc-receiver-template" id="wgc-template-<?php esc_attr_e($template) ?>" value="<?php esc_attr_e($template); ?>" required <?php checked($template, $templates[0]); ?>/>
